...
<?php get_header('sales'); 
  ...
  function increment_and_reset_count() {
   ...
   $items = get_field('calendly_urls');
   
     if (!isset($_SESSION['current_array_index'])) {
        $_SESSION['current_array_index'] = 0;
    } else {
        // Increment the index to rotate to the next item
        $_SESSION['current_array_index'] = ($_SESSION['current_array_index'] + 1) % count($items);
    }

    // Use the current session index to get the array item
    $currentItem = $items[$_SESSION['current_array_index']];
...
<div class="main-container">
    <section class="bg-img bg-top bg-overlay-left-full py-5 bg-overlay-left-full sales-banner" data-background-image="<?php bloginfo('template_directory') ?>/img/sales-banner.jpg">
        <div class="container z-1 cross-vertical-right">
            <div class="row py-lg-5 justify-content-center mb-5 pb-5">
                <div class="col-lg-9 col-xxl-10 text-center">
                    <img src="<?php bloginfo('template_directory') ?>/img/logo-white.png" alt="logo" class="logo-white mb-5" style="max-width: 15rem;" />
                    <h4 class="text-white mb-5"><?php the_content();?></h4>
                </div>
                <div class="w-100"></div>
                        ...
                        <div class="tab-pane fade bg-white show active" id="nav-profile" role="tabpanel" aria-labelledby="nav-profile-tab">
                            ...
                            <script>
                                ...
                                Calendly.initInlineWidget({
                                    url: '<?php echo $currentItem['urls']; ?>?hide_landing_page_details=1',
